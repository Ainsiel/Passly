// tests/k6/soak.js — Soak: carga sostenida 5 min sobre POST /reservations
...
function textSummary(data) {
  const lines = [];
  lines.push('');
  lines.push('=== SOAK RESULTS ===');
  lines.push(`  Total requests: ${data.metrics.http_reqs?.values?.count || 0}`);
  lines.push(`  Avg RPS:        ${(data.metrics.http_reqs?.values?.rate || 0).toFixed(1)}`);
